...
with calc_tab1:
    ...
    with col1:
        mw = st.number_input(
            "分子量 MW (g/mol)", min_value=0.000001, value=500.0, step=1.0
        )
        direction = st.radio(
            "换算方向", ["ng/mL → nmol/L", "nmol/L → ng/mL"], horizontal=True
        )
        if direction == "ng/mL → nmol/L":
            value = st.number_input(
                "输入浓度 (ng/mL)", min_value=0.0, value=100.0, step=1.0
            )
        else:
            value = st.number_input(
                "输入浓度 (nmol/L)", min_value=0.0, value=100.0, step=1.0
            )
...
with calc_tab2:
    ...
    with col1:
        dose = st.number_input(
            "Dose", min_value=0.0, value=10.0, step=0.1, key="cl_dose"
        )
...
with calc_tab3:
    ...
    with col1:
        kel = st.number_input(
            "消除速率常数 kel (1/h)", min_value=0.0, value=0.1, step=0.01
        )
    with col2:
...
with calc_tab4:
    col1, col2 = st.columns(2)
    with col1:
        cl_input = st.number_input(
            "CL", min_value=0.0, value=5.0, step=0.1, key="vd_cl"
        )
        kel_input = st.number_input(
            "kel (1/h)", min_value=0.0, value=0.1, step=0.01, key="vd_kel"
        )
    with col2:
