    ...
    try:
        ...
        if not cids or len(cids) == 0:
            return {
                "mol": None,
                "success": False,
                "message": f"RDKit构象生成失败: {original_smiles}",
            }
        ...
        return {
            "mol": None,
            "success": False,
            "message": f"异常: {original_smiles}: {str(e)}",
        }
    ...
    if file_ext == "csv":
        ...
        if isinstance(file_to_scan, str):
            ...
            if file_size > HUGE_FILE_THRESHOLD:
                ...
                preview_data, estimated_total = sample_lines_from_file(
                    file_to_scan, PREVIEW_SIZE
                )
        ...
        if isinstance(file_to_scan, str):
            if file_size > HUGE_FILE_THRESHOLD:
                ...
                if estimated_count:
                    ...
                    preview_data = [
                        Chem.MolToSmiles(m) if m else "无效分子"
                        for m in temp_preview_mols
                    ]
                    scan_successful = True
                else:
            ...
            if not scan_successful:  # 如果估算失败，使用常规方法
                ...
                for i, mol in enumerate(supplier):
                    ...
                    if file_size > LARGE_FILE_THRESHOLD and i % 1000 == 0 and i > 0:
                        ...
                        st.session_state.scan_status_placeholder.info(
                            f"⏳ 扫描进度: {i:,} 个条目 ({elapsed:.1f}秒)"
                        )

                total_mols = count
                preview_data = [
                    Chem.MolToSmiles(m) if m else "无效分子" for m in temp_preview_mols
                ]
                scan_successful = True
        else:
            # UploadedFile SDF
...
def render_file_preview_section(
    ...
    if cache_valid:
        ...
        try:
            with st.spinner(f"正在智能扫描文件 '{current_filename}'..."):
                total_mols, preview_data, df_preview, scan_successful, is_estimated = (
                    smart_file_scan(file_to_scan, current_filename, smiles_column)
                )
            ...
            if scan_successful:
                ...
                current_identifier = (
                    file_to_scan
                    if isinstance(file_to_scan, str)
                    else getattr(file_to_scan, "name", str(file_to_scan))
                )
    ...
    if scan_successful:
        ...
        if total_mols == -1:
            ...
            st.success(
                f"扫描完成: 在 '{current_filename}' 中找到 {total_mols:,} 个条目"
            )
    else:
        if input_method == "上传新文件":
...
def simple_run_background_conformer(work_dir):
    """简单的后台运行功能"""
    # 创建运行脚本
    run_script_content, run_script_name = create_simple_run_script(
        work_dir, "conformer"
    )
...
if input_method == "上传新文件":
    ...
    elif st.session_state.saved_file_path and os.path.exists(
        st.session_state.saved_file_path
    ):
        selected_file_path = st.session_state.saved_file_path
        work_dir = st.session_state.saved_work_dir
        ...
        if selected_folder:
            ...
            if not files_in_folder:
                ...
                selected_filename = st.selectbox(
                    "选择输入文件:", options=files_in_folder, index=0
                )
...
input_ready = (uploaded_file is not None) or (
    selected_file_path is not None and os.path.exists(selected_file_path or "")
)
...
with col1:
    num_conformers = st.number_input(
        "每个分子的构象数:", min_value=1, max_value=100, value=10
    )
with col2:
    max_attempts = st.number_input(
        "最大嵌入尝试次数:", min_value=1, max_value=500, value=100
    )
with col3:
    random_seed = st.number_input(
        "随机种子:", min_value=-1, value=42, help="-1表示不指定种子"
    )

# 执行模式设置
st.subheader("步骤三 · 选择执行模式")
st.markdown(
    "根据任务规模选择执行方式。后台模式支持断点恢复并提供脚本工具，前台模式适合小规模快速验证。"
)
...
if execution_mode == "智能后台执行 (推荐)":
    ...
    if num_workers > 128:
        st.info(
            "💡 **集群建议**: >128 进程通常需要多节点，请在 SLURM 脚本中配置节点信息。"
        )
    elif num_workers > 64:
...
selected_scope = st.radio(
    "选择处理范围", options=processing_options, index=0, horizontal=True
)
                ...
                if not (selected_file_path or uploaded_file):
                    ...
                    try:
                        if selected_file_path and os.path.exists(selected_file_path):
                            ...
                            if not (
                                st.session_state.saved_file_path
                                and os.path.exists(st.session_state.saved_file_path)
                            ):
                                file_path, work_dir_new = save_uploaded_file(
                                    uploaded_file
                                )
                                if file_path:
                                    st.session_state.saved_file_path = file_path
                        ...
                        script_path, script_name, script_content = (
                            generate_conformer_script(
                                input_file_path,
                                output_path,
                                num_conformers,
                                max_attempts,
...
"""
        ...
        with col_op2:
            if st.button(
                "🚀 智能后台运行", type="secondary", help="快速启动智能后台构象生成"
            ):
                if not (selected_file_path or uploaded_file):
                    st.error("❌ 请先选择输入文件")
                else:
                    try:
                        ...
                        log_file = os.path.join(
                            work_directory, f"{script_name}_output.log"
                        )
                        progress_file = os.path.join(
                            work_directory, f"{script_name}.progress"
                        )
...
'''
                        ...
                        preprocess_path = os.path.join(
                            work_directory, "preprocess_check.py"
                        )
            ...
            if st.button("🖥️ 生成SLURM脚本", help="生成用于SLURM集群的任务提交脚本"):
                if not (selected_file_path or uploaded_file):
                    ...
                    try:
                        ...
                        estimated_molecules = (
                            total_mols if (total_mols and total_mols > 0) else 100000
                        )
                        total_conformers = estimated_molecules * num_conformers
                        estimated_time_hours = max(
                            1, total_conformers / (num_workers * 5000)
                        )
...
if not input_ready:
    ...
    if total_mols in (None, 0):
        ...
        if (
            ...
            and total_mols not in (None, -1)
            and total_mols > processing_limit
        ):
            st.info(
                f"范围选择: 将处理 {total_mols:,} 个条目中的前{int(processing_limit):,}个"
            )

        if (
            st.session_state.file_size_cache
            and st.session_state.file_size_cache > LARGE_FILE_THRESHOLD
        ):
            st.warning(
                "⚡ **大文件建议**: 对于百万级分子库，建议选择较小的处理范围以获得更好的性能"
            )
        ...
        if st.button(button_label):
            ...
            with st.spinner(f"正在从 '{current_filename}' 加载和解析分子..."):
                try:
                    current_file = (
                        uploaded_file if uploaded_file else selected_file_path
                    )
                    ...
                    if file_ext == "csv":
                        if st.session_state.file_size_cache > LARGE_FILE_THRESHOLD:
                            ...
                            for chunk in pd.read_csv(
                                current_file, chunksize=chunk_size
                            ):
                                if smiles_column in chunk.columns:
                                    ...
                                    if total_processed % 50000 == 0:
                                        if not hasattr(
                                            st.session_state, "parse_status_placeholder"
                                        ):
                                            st.session_state.parse_status_placeholder = (
                                                st.empty()
                                            )
                            ...
                            st.info(
                                f"CSV完全读取完成。包含 {csv_row_count:,} 行数据。开始解析..."
                            )
                            ...
                            if smiles_column in df_full.columns:
                                ...
                                st.error(
                                    f"SMILES列 '{smiles_column}' 不存在，请重新检查。"
                                )
                                st.stop()
                        ...
                        for i, mol in enumerate(supplier):
                            ...
                            if mol is not None:
                                input_mols.append(mol)
                                source_ids.append(
                                    f"SDF分子 {i+1} ({Chem.MolToSmiles(mol)})"
                                )
            ...
            tasks = [
                (mol, num_conformers, max_attempts, random_seed) for mol in input_mols
            ]
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                future_to_idx = {
                    executor.submit(generate_conformers_for_mol, *task): i
                ...
                for future in concurrent.futures.as_completed(future_to_idx):
                    ...
                    if (
                        ...
                        or (completed % 100 == 0)
                        or (completed == len(input_mols))
                    ):
                        elapsed_time = current_time - start_time
                        progress_bar.progress(progress)

                        completed_metric.metric(
                            "已完成", f"{completed:,}/{len(input_mols):,}"
                        )
                        success_metric.metric(
                            "成功率",
                            (
                                f"{success_count/completed*100:.1f}%"
                                if completed > 0
                                else "0%"
                            ),
                        )
                        ...
                        if completed > 10:
                            ...
                            remaining_time = avg_time_per_mol * (
                                len(input_mols) - completed
                            )
                            speed = completed / elapsed_time
                            speed_metric.metric("处理速度", f"{speed:.1f} 分子/秒")
                            status_text.info(
                                f"⏱️ 预计剩余时间: {remaining_time/60:.1f} 分钟"
                            )
                        else:
                            speed_metric.metric("处理速度", "计算中...")
                            status_text.info(
                                f"🔄 处理中: {completed:,}/{len(input_mols):,} 分子完成"
                            )
            ...
            st.success(
                f"🎉 成功为 {success_count:,}/{len(input_mols):,} 个分子生成了构象！"
            )
...
with col_clear:
    if st.button("🗑️ 清理任务", help="清理已完成的任务"):
        ...
        for task in st.session_state.simple_background_tasks:
            status_info = simple_check_background_status(
                task["work_dir"], task["script_name"]
            )
...
if (
    "simple_background_tasks" in st.session_state
    and st.session_state.simple_background_tasks
):
    # 检查是否有运行中的任务
    ...
    for idx, task in enumerate(st.session_state.simple_background_tasks):
        with st.expander(f"📋 任务 {idx+1}: {task['script_name']}", expanded=True):
            ...
            with col3:
                ...
                st.metric(
                    "配置",
                    f"{config['num_workers']}进程/{config['num_conformers']}构象",
                )
...
""",
            ...
            with col_btn1:
                if st.button(f"📖 查看日志 {idx+1}", key=f"log_{idx}"):
                    log_file = os.path.join(
                        task["work_dir"], f"{task['script_name']}.log"
                    )
                ...
                if st.button(f"📊 检查结果 {idx+1}", key=f"result_{idx}"):
                    if status_info["status"] == "completed":
                        ...
                        for file in os.listdir(task["work_dir"]):
                            if (
                                file.startswith("conformers_")
                                or file.startswith("generated_conformers_")
