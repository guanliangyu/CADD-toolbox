...
if TORCH_AVAILABLE:
    if CUDA_AVAILABLE:
        ...
        gpu_info = (
            torch.cuda.get_device_name(0) if torch.cuda.is_available() else "未知"
        )
...
def calculate_distance_matrix_gpu(
    data_tensor, distance_method="euclidean", use_half_precision=False, chunk_size=None
):
    """GPU版本高效矩阵化距离计算"""
    ...
    # 混合精度优化
    compute_dtype = (
        torch.float16 if use_half_precision and AMP_AVAILABLE else torch.float32
    )
    ...
    if chunk_size is None:
        ...
        max_elements = int(
            free_memory
            * 0.7
            / (compute_dtype.itemsize if hasattr(compute_dtype, "itemsize") else 4)
        )
        chunk_size = min(N, int(np.sqrt(max_elements)))
        st.info(
            f"🧠 自动估算chunk_size: {chunk_size} (可用GPU内存: {free_memory/1024**3:.1f}GB)"
        )
...
@torch.jit.script
def _calculate_euclidean_matrix_gpu(
    data_tensor: torch.Tensor, chunk_size: int
) -> torch.Tensor:
    """优化的欧氏距离矩阵计算"""
    ...
    if N <= chunk_size:
        # 小数据集：一次性计算
        distance_matrix = (
            sq_norms + sq_norms.T - 2 * torch.mm(data_tensor, data_tensor.T)
        )
        ...
        for i in range(0, N, chunk_size):
            ...
            for j in range(0, N, chunk_size):
                ...
                # 计算块间距离
                block_dist = (
                    sq_norms_i + sq_norms_j.T - 2 * torch.mm(chunk_i, chunk_j.T)
                )
                distance_matrix[i:i_end, j:j_end] = torch.sqrt(
                    torch.clamp(block_dist, min=0)
                )
...
def calculate_cosine_distance_gpu(data_tensor, selected_indices):
    ...
    for selected_idx in selected_indices:
        similarities = torch.matmul(
            data_norm, data_norm[selected_idx].unsqueeze(1)
        ).squeeze()
...
def greedy_selection_cpu(
    data_np, subset_size, distance_method="euclidean", initial_method="random", seed=42
):
    """CPU版本贪心算法多样性筛选"""
    np.random.seed(seed)
    N = len(data_np)
...
def greedy_selection_gpu_optimized(
    data_tensor,
    subset_size,
    ...
    st.info(
        f"🚀 启动GPU优化版贪心算法 (矩阵模式: {use_matrix_mode}, 批量大小: {batch_size})"
    )

    if use_matrix_mode and N <= 20000:  # 矩阵模式适用于中等规模数据
        return _greedy_selection_matrix_mode(
            data_tensor,
            subset_size,
            distance_method,
            initial_method,
        ...
        return _greedy_selection_batch_mode(
            data_tensor,
            subset_size,
            distance_method,
            initial_method,
        ...
        return _greedy_selection_sequential_optimized(
            data_tensor, subset_size, distance_method, initial_method, seed
        )


def _greedy_selection_matrix_mode(
    data_tensor, subset_size, distance_method, initial_method, seed, use_half_precision
):
    ...
    with st.spinner("计算中..."):
        ...
        if use_half_precision and AMP_AVAILABLE:
            with autocast():
                distance_matrix = calculate_distance_matrix_gpu(
                    data_tensor, distance_method, use_half_precision
                )
        else:
            distance_matrix = calculate_distance_matrix_gpu(
                data_tensor, distance_method, use_half_precision
...
def _greedy_selection_batch_mode(
    data_tensor,
    subset_size,
    distance_method,
    initial_method,
    ...
    while len(selected_indices) < subset_size:
        ...
        # 计算到已选集合的最小距离
        min_distances = _compute_min_distances_optimized(
            data_tensor, selected_indices, distance_method, use_half_precision
        )
...
def _greedy_selection_sequential_optimized(
    data_tensor, subset_size, distance_method, initial_method, seed
):
    """优化的逐个选择模式：改进的原始算法"""
    torch.manual_seed(seed)
    ...
    for i in range(1, subset_size):
        ...
        if i == 1:
            # 第一次计算
            min_distances = _compute_min_distances_optimized(
                data_tensor, selected_indices, distance_method
            )
        else:
            # 增量更新：只计算新加点的距离
            new_point_idx = selected_indices[-1]
            new_distances = _compute_single_point_distances(
                data_tensor, new_point_idx, distance_method
            )
            min_distances = torch.minimum(min_distances, new_distances)
...
def _compute_min_distances_optimized(
    data_tensor, selected_indices, distance_method, use_half_precision=False
):
    """优化的最小距离计算"""
    len(data_tensor)
    ...
    if use_half_precision and AMP_AVAILABLE:
        with autocast():
            return _compute_min_distances_core(
                data_tensor, selected_indices, distance_method
            )
    else:
        return _compute_min_distances_core(
...
def _compute_min_distances_core(data_tensor, selected_indices, distance_method):
    ...
    if len(selected_indices) == 1:
        # 单点情况
        return _compute_single_point_distances(
            data_tensor, selected_indices[0], distance_method
        )
...
def greedy_selection_gpu(
    ...
    """向后兼容的GPU贪心选择函数"""
    return greedy_selection_gpu_optimized(
        data_tensor, subset_size, distance_method, initial_method, seed
    )


def sphere_exclusion_cpu(
    data_np, subset_size, distance_method="euclidean", radius=None, seed=42
):
    """CPU版本球体排除法多样性筛选"""
    np.random.seed(seed)
    N = len(data_np)
    ...
    if radius is None:
        ...
        if distance_method == "euclidean":
            pairwise_dists = np.sqrt(
                np.sum((sample_data[:, None] - sample_data[None, :]) ** 2, axis=2)
            )
        elif distance_method == "manhattan":
            pairwise_dists = np.sum(
                np.abs(sample_data[:, None] - sample_data[None, :]), axis=2
            )
        elif distance_method == "cosine":
            sample_norm = sample_data / (
                np.linalg.norm(sample_data, axis=1, keepdims=True) + 1e-8
            )
    ...
    while len(selected_indices) < subset_size and not np.all(excluded):
        ...
        if distance_method == "euclidean":
            ...
            data_norm = data_np / (
                np.linalg.norm(data_np, axis=1, keepdims=True) + 1e-8
            )
...
def sphere_exclusion_gpu(
    data_tensor, subset_size, distance_method="euclidean", radius=None, seed=42
):
    """GPU版本球体排除法多样性筛选"""
    torch.manual_seed(seed)
    device = data_tensor.device
    ...
    while len(selected_indices) < subset_size and not torch.all(excluded):
        ...
        if distance_method == "euclidean":
            distances = torch.sqrt(
                torch.sum((data_tensor - data_tensor[current_idx]) ** 2, dim=1)
            )
        elif distance_method == "manhattan":
            distances = torch.sum(
                torch.abs(data_tensor - data_tensor[current_idx]), dim=1
            )
        elif distance_method == "cosine":
            data_norm = torch.nn.functional.normalize(data_tensor, p=2.0, dim=1)
            current_norm = data_norm[current_idx]
    ...
    if selected_file:
        ...
        with col1:
            ...
            if TORCH_AVAILABLE and CUDA_AVAILABLE:
                use_gpu = st.checkbox(
                    "使用GPU加速", value=True, help="启用GPU可显著加速大规模数据的筛选"
                )
                ...
                if use_gpu:
                    ...
                    auto_chunk = st.checkbox(
                        "自动内存优化", value=True, help="根据GPU内存自动调整块大小"
                    )
                    if not auto_chunk:
        ...
        with col2:
            ...
            if selection_algorithm == "sphere_exclusion":
                auto_radius = st.checkbox(
                    "自动估计排除半径", value=True, help="根据数据分布自动确定排除半径"
                )
                if not auto_radius:
        ...
        if st.button("🚀 开始多样性筛选", type="primary"):
            try:
                ...
                st.info(
                    f"🔢 找到 {len(numeric_cols)} 个数值描述符列，{len(non_numeric_cols)} 个非数值列"
                )
                ...
                if subset_method == "按比例":
                    final_subset_size = max(
                        1, int(total_molecules * subset_ratio / 100)
                    )
                else:
                    final_subset_size = min(subset_size, total_molecules)

                st.info(
                    f"🎯 将从 {total_molecules} 个分子中选择 {final_subset_size} 个代表性分子"
                )
                ...
                with st.spinner("执行多样性筛选..."):
                    ...
                    if use_gpu and TORCH_AVAILABLE and CUDA_AVAILABLE:
                        ...
                        data_tensor = torch.tensor(
                            descriptor_data.values, dtype=compute_dtype, device=device
                        )

                        st.info(
                            f"📊 数据形状: {data_tensor.shape}, 类型: {data_tensor.dtype}"
                        )
                        ...
                        st.success(
                            f"✅ GPU内存使用情况 - 峰值: {peak_memory:.2f}GB, 当前: {final_memory:.2f}GB"
                        )
                ...
                with col3:
                    st.metric(
                        "筛选比例", f"{len(diverse_subset_df)/total_molecules*100:.1f}%"
                    )
                with col4:
                    # 计算处理速度
                ...
                if use_gpu and TORCH_AVAILABLE and CUDA_AVAILABLE:
                    ...
                    performance_info.append(
                        f"🎮 **GPU设备**: {torch.cuda.get_device_name(0)}"
                    )
                ...
                with param_col1:
                    st.text(
                        f"计算设备: {'GPU' if use_gpu and TORCH_AVAILABLE and CUDA_AVAILABLE else 'CPU'}"
                    )
