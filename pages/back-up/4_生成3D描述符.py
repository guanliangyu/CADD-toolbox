...
# 导入后台描述符计算工具
try:
    from utils.background_descriptor import run_background_descriptor_calculation, check_background_descriptor_status
...
def create_batch_files(molecules_by_id, batch_size, output_dir, base_filename):
    ...
    st.info(f"🔄 开始创建 {(len(mol_ids) + batch_size - 1) // batch_size} 个批次文件...")
    ...
    for batch_idx in range(0, len(mol_ids), batch_size):
        ...
        # 记录批次信息
        st.info(f"📦 创建批次 {batch_num}: {batch_filename} ({mol_count} 个分子, {conformer_count} 个构象)")
    ...
    try:
        ...
        for batch_file in batch_result_files:
            if os.path.exists(batch_file):
                ...
                st.info(f"✅ 批次结果: {os.path.basename(batch_file)} ({len(df)} 个分子)")
            else:
        ...
        if combined_dfs:
            ...
            st.success(f"✅ 合并完成! 总计 {total_molecules} 个分子, 文件大小: {file_size:.1f} MB")
...
def process_batch_intelligently(batch_files, output_dir, config):
    ...
    for i, batch_file in enumerate(batch_files):
        batch_num = i + 1
        st.info(f"🔄 启动批次 {batch_num}/{len(batch_files)}: {os.path.basename(batch_file)}")
        
        try:
            st.info(f"🔄 正在启动批次 {batch_num}，输入文件: {os.path.basename(batch_file)}")
...
def check_all_batches_completed(batch_files, output_dir):
    ...
    for i, batch_file in enumerate(batch_files):
        batch_num = i + 1
        expected_result = os.path.join(output_dir, f"descriptors_batch_{batch_num:03d}.csv")
            ...
            if selected_file:
                ...
                if st.button("🚀 开始计算描述符", type="primary"):
                    ...
                    for task in st.session_state.background_descriptor_tasks:
                        if BACKGROUND_DESCRIPTOR_AVAILABLE:
                            status_info = check_background_descriptor_status(task['output_dir'], task['script_name'])
                            if status_info['status'] == 'running':
                                running_tasks.append(task)
                    
                    if running_tasks:
                        st.warning(f"⚠️ 已有 {len(running_tasks)} 个任务正在运行中！")
                        st.info("请在下方监控区域查看进度，或先停止当前任务再启动新任务。")
                    ...
                    try:
                        ...
                        need_batch_processing = len(molecules) > BATCH_SIZE and processing_limit == float('inf')
                        
                        if need_batch_processing:
                            st.warning(f"🔄 检测到大化合物库 ({len(molecules)} 个分子)，将拆分成批次处理")
                            st.info(f"📦 每批处理 {BATCH_SIZE:,} 个分子，共需 {(len(molecules) + BATCH_SIZE - 1) // BATCH_SIZE} 批次")
                            
                            # 询问用户是否继续
                            proceed_batch = st.button("✅ 确认开始批次处理", type="primary", key="confirm_batch")
                        ...
                        # 确定输出目录
                        abs_work_dir = os.path.abspath(os.path.join(DATA_DIR, selected_folder))
                        ...
                        if need_batch_processing:
                            ...
                            try:
                                ...
                                batch_files = create_batch_files(
                                    molecules_by_id, BATCH_SIZE, abs_work_dir, base_filename
                                )
                                ...
                                batch_tasks = process_batch_intelligently(batch_files, abs_work_dir, batch_config)
                                ...
                                for task in batch_tasks:
                                    st.session_state.background_descriptor_tasks.append(task)
                                ...
                                st.info("当所有批次完成后，可以使用以下Python代码合并结果：")
...
"""
                            ...
                            try:
                                ...
                                st.info(f"⚙️ 配置: {num_workers}进程, {processing_limit}分子, {aggregation_method}聚合")
                                ...
                                # 显示日志文件路径
                                log_file = os.path.join(abs_work_dir, f"{script_name}_output.log")
                                st.info(f"📝 日志文件: {log_file}")
                                ...
                                st.session_state.background_descriptor_tasks.append(task_info)
    ...
    if st.session_state.background_descriptor_tasks:
        for idx, task in enumerate(st.session_state.background_descriptor_tasks):
            with st.expander(f"📋 任务 {idx+1}: {task['script_name']}", expanded=True):
                ...
                with col3:
                    config = task['config']
                    st.metric("配置", f"{config['num_workers']}进程/{config['processing_limit']}分子")
                    ...
                    if st.button(f"📊 检查结果 {idx+1}", key=f"desc_result_{idx}"):
                        if status_info['status'] == 'completed':
                            ...
                            for file in os.listdir(task['output_dir']):
                                if file.startswith('descriptors_') and file.endswith('.csv'):
