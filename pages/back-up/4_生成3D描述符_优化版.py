    ...
    if selected_file:
        ...
        with col1:
            ...
            processing_option = st.selectbox(
                "选择处理范围:",
                ["处理所有分子", "仅处理前10,000个分子（快速测试）", "仅处理前1,000个分子（功能测试）"],
                index=0
            )
        ...
        if st.button("🚀 执行脚本", type="primary", key="main_execute"):
            if os.path.exists(script_info['shell_script_path']):
                ...
                try:
                    ...
                    st.info(f"📄 执行脚本: {os.path.basename(script_info['shell_script_path'])}")
                    st.info(f"📝 日志文件: {os.path.basename(script_info['stdout_log'])}")
...
if 'running_processes' in st.session_state and st.session_state.running_processes:
    ...
    for i, process_info in enumerate(st.session_state.running_processes):
        with st.expander(f"进程 {i+1}: {process_info['script_name']} (PID: {process_info['pid']})", expanded=True):
            col1, col2, col3 = st.columns(3)
            ...
            if os.path.exists(output_file):
                file_size = os.path.getsize(output_file) / (1024*1024)
                st.success(f"✅ 输出文件已存在: {os.path.basename(output_file)} ({file_size:.1f} MB)")
...
if log_dirs:
    selected_log_folder = st.selectbox(
        "选择数据文件夹:",
        [folder for folder, _ in log_dirs],
