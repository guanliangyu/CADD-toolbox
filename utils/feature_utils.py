    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if gpu_config.get("enabled", False) and GPU_TOOLS_AVAILABLE:
            ...
            if gpu_config.get("auto_detect", True):
                ...
                self.use_gpu = gpu_status["any_gpu"] and gpu_config.get(
                    "features", {}
                ).get("pca", True)
            else:
                self.use_gpu = True
    ...
    def _fit_transform_pca(self, features: np.ndarray) -> np.ndarray:
        ...
        if self.use_gpu:
            try:
                ...
                logger.info(
                    f"使用GPU进行PCA: 选择了{self.n_components}个主成分，解释了{sum(explained_variance_ratio):.2%}的方差"
                )
        ...
        logger.info(
            f"CPU PCA降维: 选择了{self.n_components}个主成分，解释了{explained_var_ratio:.2%}的方差"
        )
...
class FeatureCombiner:
    ...
    def combine_features(
        ...
        for i in range(n_mols):
            ...
            # 添加指纹特征
            if (
                fp_features is not None
                and i < len(fp_features)
                and fp_features[i] is not None
            ):
                mol_features.extend(fp_features[i])

            # 添加基本描述符特征
            if (
                basic_features is not None
                and i < len(basic_features)
                and basic_features[i]
            ):
                # 获取配置中指定的描述符
            ...
            # 添加形状描述符特征
            if (
                shape_features is not None
                and i < len(shape_features)
                and shape_features[i]
            ):
                ...
                if shape_config.get("enabled", True):
                    ...
                    for shape_prop in selected_shape:
                        if (
                            shape_prop == "moments"
                            and "principal_moment_1" in shape_features[i]
                        ):
            ...
            # 添加电荷特征
            if (
                charges_features is not None
                and i < len(charges_features)
                and charges_features[i]
            ):
                ...
                if gpu_status["faiss_gpu"]:
                    try:
                        ...
                        if hasattr(reference_fps[0], "__len__"):
                            self.reference_array = np.array(
                                [list(fp) for fp in reference_fps], dtype=np.float32
                            )
                            # 创建FAISS索引
                            self.index = faiss.IndexFlatL2(
                                self.reference_array.shape[1]
                            )
                            ...
                            logger.info(
                                f"FAISS GPU索引已创建: {self.reference_array.shape}"
                            )
                    except Exception as e:
