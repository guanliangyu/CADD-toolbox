...
def plot_property_distributions(
    ...
    try:
        ...
        for idx, prop in enumerate(properties):
            ...
            try:
                ...
                if prop in df.columns:
                    ...
                    # 添加KDE曲线
                    sns.kdeplot(data=df, x=prop, ax=ax, color="blue", linestyle="--")
                    if subset_df is not None and prop in subset_df.columns:
                        sns.kdeplot(
                            data=subset_df, x=prop, ax=ax, color="red", linestyle="--"
                        )
                    ...
                    ax.text(0.5, 0.5, f"属性 {prop} 不存在", ha="center", va="center")
                    ax.set_axis_off()
        ...
        # 返回错误提示图
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f"绘制属性分布图失败: {str(e)}", ha="center", va="center")
        return fig
...
def plot_fps_pca(
    ...
    try:
        ...
        # 返回错误提示图
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f"PCA可视化失败: {str(e)}", ha="center", va="center")
        return fig
...
def plot_fps_tsne(
    ...
    try:
        ...
        # 执行t-SNE降维
        tsne_params = {"n_components": 2, "perplexity": perplexity, "random_state": 42}
        ...
        # 返回错误提示图
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f"t-SNE可视化失败: {str(e)}", ha="center", va="center")
        return fig
...
def plot_fps_umap(
    ...
    try:
        ...
        # 返回错误提示图
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f"UMAP可视化失败: {str(e)}", ha="center", va="center")
        return fig
...
def plot_dimensionality_reduction(
    ...
    try:
        ...
        ax.text(0.5, 0.5, f"Visualization failed: {str(e)}", ha="center", va="center")
        return fig
...
def plot_similarity_distribution(
    similarity_matrix: np.ndarray, figsize: Tuple[int, int] = (10, 6)
) -> plt.Figure:
    """
