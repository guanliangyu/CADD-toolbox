...
def calculate_distance_matrix(
    fps: List[Any], metric: str = "tanimoto", config: Optional[Dict[str, Any]] = None
) -> np.ndarray:
    """
    计算距离矩阵
    ...
    if config is not None and GPU_TOOLS_AVAILABLE:
        ...
        if gpu_config.get("enabled", False):
            ...
            if gpu_config.get("auto_detect", True):
                ...
                use_gpu = gpu_status["any_gpu"] and gpu_config.get("features", {}).get(
                    "distances", True
                )
            else:
                use_gpu = True
    ...
    try:
        ...
        if hasattr(fps[0], "__len__"):  # 检查是否可迭代
            features = np.array(
                [list(fp) for fp in fps if fp is not None], dtype=np.float32
            )
    ...
    if use_gpu:
        try:
            ...
            distance_matrix = calculate_distance_matrix(
                fps, metric=distance_metric, config=config
            )
...
def kmeans_clustering(
    ...
    if use_gpu:
        try:
            ...
            gpu_kmeans = GPUKMeans(
                n_clusters=n_clusters, random_state=42, max_iter=max_iter
            )
    ...
    if len(features) < 10000:
        ...
        kmeans = KMeans(
            n_clusters=n_clusters, random_state=42, max_iter=max_iter, n_init=10
        )
    else:
...
def maxmin_selection(
    ...
    if use_gpu:
        try:
            ...
            selected_indices = list(
                picker.LazyPick(distance_fn, n, num_to_select, seed_idx)
            )
    ...
    selected_indices = list(picker.LazyPick(distance_fn, n, num_to_select, seed_idx))
...
def hdbscan_clustering(
    ...
    if config is not None and GPU_TOOLS_AVAILABLE:
        ...
        if gpu_config.get("enabled", False):
            ...
            if gpu_config.get("auto_detect", True):
                gpu_status = check_gpu_availability()
                use_gpu = gpu_status["cuml"] and gpu_config.get("features", {}).get(
                    "kmeans", True
                )
            else:
                use_gpu = True
    ...
    if use_gpu and method == "centroid":
        try:
            ...
            if hasattr(fps[0], "__len__"):
                ...
                distance_matrix = calculate_distance_matrix(
                    features, metric="tanimoto", config=config
                )
        except Exception as e:
...
def select_representatives_from_kmeans(
    features: np.ndarray,
    labels: np.ndarray,
    centers: np.ndarray,
    config: Optional[Dict[str, Any]] = None,
...
def perform_clustering(
    fps: List[Any], method: str, config: Dict[str, Any]
) -> Dict[str, Any]:
    """
    执行聚类分析
    ...
    try:
        if method == "butina":
            ...
            batch_size = (
                config.get("data", {}).get("batching", {}).get("batch_size", 1000)
            )
            ...
            init_method = (
                ...
                .get("maxmin", {})
                .get("init_method", "random")
            )
            seed_idx = 0 if init_method == "first" else -1

            # 执行MaxMin选择
            selected_indices = maxmin_selection(
                valid_fps, num_to_select=num_to_select, seed_idx=seed_idx, config=config
            )
...
def evaluate_clustering(
    features: np.ndarray, labels: np.ndarray, method: str
) -> Dict[str, float]:
    """
    评估聚类结果的质量
        ...
        if method == "kmeans":
            ...
            if len(selected_points) > 0:
                ...
                for point in features:
                    min_dist = np.min(
                        [
                            np.linalg.norm(point - sel_point)
