...
def create_config_from_parameters(
    ...
    config = {
        "data": {
            "filtering": {"enabled": True, "max_mw": 1000, "min_mw": 100},
            "conformers": {"enabled": include_3d},
            "charges": {"enabled": include_charges if include_3d else False},
            "batching": {"enabled": True, "batch_size": batch_size, "n_jobs": n_jobs},
        },
        "features": {
            "fingerprints": {
                "types": [fps_type],
        ...
        "clustering": {
            ...
            "butina": {"cutoff": cutoff if clustering_method == "butina" else 0.4},
            "kmeans": {
                "n_clusters": (
                    n_clusters
                    if clustering_method == "kmeans" and use_fixed_clusters
                    else 100
                ),
            ...
            "maxmin": {
                "init_method": (
                    init_method if clustering_method == "maxmin" else "random"
                )
            },
        },
    ...
    if use_gpu:
        if gpu_config is None:
            gpu_config = {
                ...
                "device_id": 0,
                "auto_detect": True,
                "features": {"kmeans": True, "distances": True, "pca": True},
            }
...
def render_clustering_parameters():
    ...
    clustering_method = st.sidebar.selectbox(
        "选择聚类/选择算法", ["butina", "kmeans", "maxmin"], key="clustering_method"
    )
    ...
    if clustering_method == "butina":
        ...
        use_fixed_clusters = st.sidebar.checkbox(
            "使用固定簇数量", value=False, key="use_fixed_clusters"
        )
        params["use_fixed_clusters"] = use_fixed_clusters

        if use_fixed_clusters:
            n_clusters = st.sidebar.number_input(
                "簇数量", 10, 10000, 100, key="n_clusters"
            )
...
def render_feature_parameters():
    ...
    fps_type = st.sidebar.selectbox(
        "指纹类型", ["morgan", "rdkit", "maccs"], key="fps_type"
    )

    params = {"fps_type": fps_type}

    if fps_type == "morgan":
        morgan_radius = st.sidebar.slider("Morgan半径", 1, 4, 2, key="morgan_radius")
        morgan_bits = st.sidebar.selectbox(
            "Morgan位数", [512, 1024, 2048], key="morgan_bits"
        )
    ...
    if include_3d:
        include_charges = st.sidebar.checkbox(
            "计算Gasteiger电荷", value=False, key="include_charges"
        )
...
def render_performance_parameters():
    ...
    n_jobs = st.sidebar.slider(
        "并行作业数", 1, mp.cpu_count(), min(4, mp.cpu_count()), key="n_jobs"
    )

    params = {"batch_size": batch_size, "n_jobs": n_jobs}
    ...
    if GPU_TOOLS_AVAILABLE:
        ...
        if use_gpu:
            ...
            if gpu_status["any_gpu"]:
                ...
                gpu_kmeans = st.sidebar.checkbox(
                    "K-means聚类", value=True, key="gpu_kmeans"
                )
                gpu_distances = st.sidebar.checkbox(
                    "距离计算", value=True, key="gpu_distances"
                )
...
def render_sidebar_parameters():
    ...
    subset_ratio = st.sidebar.slider(
        "子集比例 (%)", 0.1, 10.0, 1.0, 0.1, key="subset_ratio"
    )
