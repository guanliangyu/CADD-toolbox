...
def gpu_pca(
    ...
    if _CUML_AVAILABLE:
        try:
            ...
            logger.info(
                f"使用CUML GPU完成PCA降维，解释方差: {sum(explained_variance_ratio_cpu):.2%}"
            )
    ...
    logger.info(
        f"使用CPU完成PCA降维，解释方差: {sum(pca.explained_variance_ratio_):.2%}"
    )
        ...
        if _FAISS_GPU_AVAILABLE:
            try:
                ...
                for i in range(0, n_samples, batch_size):
                    ...
                    batch_distances = inner_product.pairwise_distances(
                        X_float32[i:end_i]
                    )
