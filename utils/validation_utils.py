...
def calculate_nearest_neighbor_distance(
    query_fps: List[Any], ref_fps: List[Any], metric: str = "tanimoto"
...
def plot_property_distributions(
    full_props: List[Dict[str, float]],
    ...
    for i, prop in enumerate(prop_names):
        if i < len(axes):
            ...
            full_values = [
                v for v in full_values if not np.isnan(v) and not np.isinf(v)
            ]
            subset_values = [
                v for v in subset_values if not np.isnan(v) and not np.isinf(v)
            ]

            # Plot histograms
            ax = axes[i]
            sns.histplot(
                full_values, ax=ax, alpha=0.5, label="Full Library", color="blue"
            )
            ...
            stats_text = (
                f"Full: μ={full_mean:.2f}, σ={full_std:.2f}\n"
...
def plot_nearest_neighbor_histogram(
    nn_distances: np.ndarray, figsize: Tuple[int, int] = (10, 6)
) -> plt.Figure:
    """
    ...
    ax.axvline(
        median_dist, color="green", linestyle="--", label=f"Median: {median_dist:.3f}"
    )
...
def plot_pca_visualization(
    ...
    if n_components >= 3:
        ...
        ax.scatter(
            full_pca[sample_idx, 0],
            full_pca[sample_idx, 1],
            full_pca[sample_idx, 2],
            alpha=0.2,
            s=10,
            label="Full Library",
        )
...
def calculate_coverage_metrics(
    ...
    nn_distances, mean_dist, max_dist, median_dist = (
        calculate_nearest_neighbor_distance(full_fps, subset_fps, metric=metric)
    )
