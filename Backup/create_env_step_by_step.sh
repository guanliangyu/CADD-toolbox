...
mamba install numpy pandas scipy scikit-learn numba -y
...
# Step 9: 安装RAPIDS基础依赖
echo "🔧 Step 9: 安装RAPIDS基础依赖"
if [ "$USE_CPU_ONLY" != "true" ]; then
    echo "安装RAPIDS基础组件..."
    mamba install -c rapidsai -c nvidia libucxx ucxx -y
    echo "✅ RAPIDS基础依赖安装完成"
else
...
# Step 11: 安装其他GPU加速库
echo "⚡ Step 11: 安装其他GPU加速库"
if [ "$USE_CPU_ONLY" != "true" ]; then
    mamba install -c nvidia cupy -y
    echo "✅ CuPy安装完成"
else
    echo "⚠️  跳过GPU加速库安装（CPU模式）"
fi
...
mamba install streamlit tqdm psutil dask hdbscan umap-learn watchdog -y
...
echo "  2. 运行测试: python test/check_jax.py (如果存在)"
