...
def optimize_single_molecule(args):
    ...
    try:
        ...
        if mp is None:
            return {'orig_idx': orig_idx, 'mol': None, 'mol_props': None, 'success': False, 'message': 'MMFF94力场初始化失败'}
        
        ff = AllChem.MMFFGetMoleculeForceField(mol_copy, mp, confId=0)
        if ff is None:
            return {'orig_idx': orig_idx, 'mol': None, 'mol_props': None, 'success': False, 'message': 'MMFF94力场创建失败'}
        ...
        print(f"分子 {orig_idx}: success={result['success']}, mol存在={result['mol'] is not None}, props存在={result['mol_props'] is not None}")
        return result
        
    except Exception as e:
        return {'orig_idx': orig_idx, 'mol': None, 'mol_props': None, 'success': False, 'message': f'异常: {str(e)}'}
...
with Pool(processes=1) as pool:
    for result_data in pool.imap_unordered(optimize_single_molecule, process_args):
        ...
        # 模拟后台脚本的判断
        condition = result_data['success'] and result_data['mol'] and result_data['mol_props']
