...
def main() -> int:
    try:
        ...
        print(
            "Tip: JAX is optional for this project, skip this check if you do not use it."
        )
        return 0
    ...
    for i, device in enumerate(jax.devices()):
        print(
            f"  Device {i}: {device.device_kind} ({device.platform}) - ID: {device.id}"
        )
    ...
    try:
        ...
        if gpu_devices_tf:
            print(
                f"SUCCESS: TensorFlow found the following GPU devices: {gpu_devices_tf}"
            )
