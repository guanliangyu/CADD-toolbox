...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("molecular_subset.log")],
)
...
def process_molecule_batch(batch_data, config, include_3d=True, include_charges=True):
    ...
    for smiles in smiles_list:
        ...
        fp = processor.compute_fingerprint(
            mol, radius=radius, nBits=n_bits, fp_type=fp_type
        )
...
def batch_process_molecules(df, config, smiles_col="SMILES"):
    ...
    logger.info(
        f"开始批量处理分子: 批量大小={batch_size}, CPU核心数={n_jobs}, 3D={include_3d}, 电荷={include_charges}"
    )
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as executor:
        for batch_result in tqdm(
            executor.map(worker, batches), total=len(batches), desc="处理分子批次"
        ):
            # 合并结果
    ...
    logger.info(
        f"分子处理完成: 总计={total}, 有效={valid_count}, 耗时={duration:.1f}秒"
    )
...
def combine_features_and_reduce(processed_results, config):
    ...
    # 记录特征维度
    logger.info(
        f"特征维度: 原始={combined_features.shape}, 降维后={reduced_features.shape}"
    )
...
def select_representative_subset(features, mols, fps, config):
    ...
    if method == "butina":
        ...
        valid_fp_idx = [
            i for i in valid_idx if fps[i] is not None and np.asarray(fps[i]).size > 0
        ]
        if not valid_fp_idx:
        ...
        # 选择最接近簇中心的分子
        rep_local_indices = select_representatives_from_kmeans(
            valid_features, labels, centers
        )
        ...
        distance_metric = clustering_config.get("maxmin", {}).get(
        ...
        # 基于密度的聚类
        min_cluster_size = clustering_config.get("hdbscan", {}).get(
            "min_cluster_size", 5
        )
...
def validate_subset(full_results, subset_indices, config):
    ...
    logger.info(
        f"覆盖度指标: 覆盖率={metrics['coverage_ratio']:.2%}, 平均距离={metrics['mean_distance']:.3f}"
    )
    ...
    if validation_config.get("property_coverage", True):
        ...
        # 绘制属性分布对比图
        prop_fig = plot_property_distributions(
            valid_full_props, subset_basic_desc, prop_names
        )
    ...
    if validation_config.get("nearest_neighbor_analysis", True):
        # 计算全库到子集的最近邻距离
        nn_distances, mean_dist, max_dist, median_dist = (
            calculate_nearest_neighbor_distance(valid_full_fps, subset_fps)
        )
...
def save_results(
    df, subset_indices, processed_results, validation_results, output_dir, config
):
    """
    保存处理结果
    ...
    if validation_results:
        ...
        if "property_distribution_fig" in validation_results:
            prop_fig_path = os.path.join(
                plots_dir, f"property_distribution_{timestamp}.png"
            )
            validation_results["property_distribution_fig"].savefig(
                prop_fig_path, dpi=300
            )
            logger.info(f"已保存属性分布图至 {prop_fig_path}")

        if "nn_distribution_fig" in validation_results:
            nn_fig_path = os.path.join(
                plots_dir, f"nearest_neighbor_distribution_{timestamp}.png"
            )
...
def main():
    ...
    parser.add_argument(
        "--config", default="configs/default_config.yml", help="配置文件路径"
    )
    ...
    subset_indices = select_representative_subset(
        reduced_features, processed_results["mols"], processed_results["fps"], config
    )
    ...
    # 保存结果
    save_results(
        df, subset_indices, processed_results, validation_results, args.output, config
    )
